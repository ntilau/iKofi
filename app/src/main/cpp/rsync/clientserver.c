...
/* IKOFI_REQUIRED_CHANGE: disable root/multi-user related code. */
#if 0
...
#endif
/* IKOFI_REQUIRED_CHANGE: is this actually needed/useful?
 * i.e. the conf file is only used when running as a daemon.
 */
		char *tmp = getenv(IKOFI_CONF_DIR);
		if (!tmp) {
			config_file = RSYNCD_SYSCONF;
		} else {
